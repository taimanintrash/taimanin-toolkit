...
function joinVoiceMetadata(u) {
  ...
  for (const m of meta) {
    ...
    u.voices.push({
      entry: null, form: u.form, type: m.type, variant: m.variant || 'base',
...
function renderVoices(u) {
  ...
  const keys = [...g.keys()].sort((a, b) => {
    const i = VOICE_ORDER.indexOf(a), j = VOICE_ORDER.indexOf(b);
...
async function openScene(k) {
  ...
  if (s.spineModel) {
    S.adv = {
      s, ops: [], pair: new Map(), locals: [], i: 0, stops: [], log: [],
  ...
  S.adv = {
    s, ops, pair, locals, i: 0, stops: [], cgStops: [], log: [],
...
async function runInner(A) {
  ...
  while (A.i < A.ops.length) {
    ...
    switch (o.t) {
      ...
      case 'SE_PLAY': {
        const e = await resolve(a[0], A.locals);
